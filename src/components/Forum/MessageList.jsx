  ...
  return (
    <div
      ...
      style={{
        ...
        overflowY: "auto",
        display: "flex",
        flexDirection: "column", // Mensajes en orden natural
      }}
    >
      {messages.map((msg, index) => {
        const isOwnMessage = msg.sender.email === user?.email; // Verifica si el mensaje es del usuario actual
        const isLastMessage = index === messages.length - 1; // Verifica si es el último mensaje

        return (
          <div
            key={index}
            ref={isLastMessage ? lastMessageRef : null} // Asigna el ref al último mensaje
            style={{
              display: "flex",
          ...
          >
            <div
              style={{
                ...
                textAlign: "left",
                wordWrap: "break-word", // Ajusta el texto al ancho del contenedor
                whiteSpace: "pre-wrap", // Maneja saltos de línea y espacios
              }}
            >
              ...
              >
                {msg.sender.rol?.name !== ROLES.Student && (
                  ...
                  >
                    <Tag color={roleColors[msg.sender.rol?.name]}>
                      {msg.sender.rol?.name.toUpperCase()}
                    </Tag>
                  </span>
                )}
                <span
